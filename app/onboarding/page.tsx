...
export default function OnboardingPage() {
  ...
  const handleFinish = async () => {
    ...
    const res = await fetch("/api/auth/update", {
      ...
      body: JSON.stringify({ full_name: name.trim(), use_case: useCase, onboarding_complete: true }),
    });
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <p className="text-sm text-ivy-text-muted mb-5">This appears across your workspace.</p>
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                  ...
                  {useCases.map((uc) => (
                    <button key={uc.id} onClick={() => setUseCase(uc.id)}
                      className={`flex flex-col items-start gap-1.5 p-3 rounded-xl border text-left transition-all duration-150 ${
                        useCase === uc.id ? "border-ivy-green bg-ivy-green/10 text-ivy-text" : "border-ivy-border bg-ivy-dark text-ivy-text-muted hover:border-ivy-border-light hover:text-ivy-text"
                      }`}>
                ...
                <button onClick={() => setStep(0)} className="w-full text-xs text-ivy-text-muted hover:text-ivy-text transition-colors mt-3">← Back</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
